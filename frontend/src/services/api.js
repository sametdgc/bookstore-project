...
export const placeOrder = async (orderDetails, orderItems) => {
  try {
    ...
    for (const item of orderItems) {
      const stockUpdateSuccess = await subtractItemsFromStock(
        item.book_id,
        item.quantity
      );
    ...
    if (!clearCartResult.success) {
      throw new Error(
        "Failed to clear the cart items after placing the order."
      );
    }

    console.log(
      "Cart items cleared successfully for user_id:",
...
export const subtractItemsFromStock = async (bookId, quantity) => {
  try {
    ...
    if (error) {
      console.error(
        `Error subtracting stock for book_id ${bookId}:`,
        error.message
      );
      return false;
    }
    ...
    console.error(
      `Unexpected error subtracting stock for book_id ${bookId}:`,
      error.message
    );
    return false;
  }
